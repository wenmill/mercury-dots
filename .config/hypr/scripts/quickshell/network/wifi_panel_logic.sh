...
# --rescan no: without it this call could trigger a FULL WiFi scan (~6.5s) —
# and the popup polls this script every 3s, so scans ran back-to-back. The
# list call below already used --rescan no; NM background-scans on its own.
CURRENT_RAW=$(LC_ALL=C nmcli -t -f active,ssid,signal,security device wifi list --rescan no | awk -F: '$1=="yes"{print; exit}')
