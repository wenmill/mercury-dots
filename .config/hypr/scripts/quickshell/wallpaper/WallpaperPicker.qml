...
Item {
    ...
    function applyWallpaper(safeFileName, isVideo) {
        ...
        if (window.currentFilter === "Search" && window.hasSearched) {
            ...
            if (alreadyExists) {
                const applyScript = `
                    ...
                    magick "$DEST_FILE" ${paths.getCacheDir("wallpaper_picker")}/current_wallpaper.png 2>/dev/null || cp "$DEST_FILE" ${paths.getCacheDir("wallpaper_picker")}/current_wallpaper.png || true
                    pkill mpvpaper || true
        ...
        const fullScript = `
            magick "${isVideo ? escThumb : escOriginal}" ${paths.getCacheDir("wallpaper_picker")}/current_wallpaper.png 2>/dev/null || cp "${isVideo ? escThumb : escOriginal}" ${paths.getCacheDir("wallpaper_picker")}/current_wallpaper.png || true
            pkill mpvpaper || true
    ...
    ListView {
        ...
        delegate: Item {
            ...
            // parent detaches while list delegates are recycled — unguarded, this
            // logged a TypeError per destroyed card.
            ...
            Item {
                ...
                Item {
                    ...
                    Image {
                        ...
                        // Without this a 4K source decodes to ~33MB RAM *per visible
                        // thumbnail*; capping to the delegate box keeps it to ~1MB each.
