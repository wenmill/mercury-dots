...
# ($HOME, not "~" — tilde does not expand inside quotes, which made the python
# flattener below silently no-op on FileNotFoundError since day one)
...
TEXT_FILES=(
    ...
    "$HOME/.config/hypr/config/colors.conf"
)
...
# Reload Kitty instances (only if running, like cava below)
