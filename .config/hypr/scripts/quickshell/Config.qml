...
Item {
    ...
    // Key and value travel as argv (never spliced into shell syntax), so
    // user-typed strings — quotes, $(), pipes — can't break out. jq gets them
    // via --arg/--argjson. [ -s ] also covers the empty-file case, where the
    // old [ ! -f ] guard let jq choke on zero-byte input.
    function setSetting(key, value) {
        rawSettings[key] = value;
        Quickshell.execDetached(["bash", "-c",
            'mkdir -p "$(dirname "$3")" && { [ -s "$3" ] || echo "{}" > "$3"; }; ' +
            'jq --arg k "$1" --argjson v "$2" \'.[$k] = $v\' "$3" > "$3.tmp" && mv "$3.tmp" "$3"',
            "qs-setting", key, JSON.stringify(value), settingsJsonPath]);
    }

    function updateJsonBulk(dataObj) {
        for (let key in dataObj) rawSettings[key] = dataObj[key];
        Quickshell.execDetached(["bash", "-c",
            'mkdir -p "$(dirname "$2")" && { [ -s "$2" ] || echo "{}" > "$2"; }; ' +
            'jq --argjson p "$1" \'. + $p\' "$2" > "$2.tmp" && mv "$2.tmp" "$2"',
            "qs-setting", JSON.stringify(dataObj), settingsJsonPath]);
    }
    ...
    // Regenerate the whole file from rawEnvs (fully read at boot) instead of
    // per-key sed edits: the old sed ran inside a double-quoted shell region,
    // so a $() or | typed into the API-key field would execute/corrupt. The
    // content now travels as argv into printf. chmod 600: the file holds keys.
    function updateEnvBulk(filePath, envDict) {
        for (let key in envDict) rawEnvs[key] = envDict[key];
        let content = "";
        for (let key in rawEnvs) {
            if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) continue;
            content += key + "='" + String(rawEnvs[key]).replace(/'/g, "'\\''") + "'\n";
        }
        Quickshell.execDetached(["bash", "-c",
            'mkdir -p "$(dirname "$2")" && printf %s "$1" > "$2" && chmod 600 "$2"',
            "qs-env", content, filePath]);
    }
