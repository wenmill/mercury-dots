...
Item {
   ...
   // -------------------------------------------------------------------------
    ...
    // Timer-stepped at 20fps instead of a per-frame NumberAnimation: the ambient
    // blobs sweep one revolution per 120s — sub-pixel per 50ms step — and the
    // render loop wakes 20x/s instead of every vsync while the popup is open.
    property real globalOrbitAngle: 0
    Timer {
        interval: 50; repeat: true; running: window.visible
        onTriggered: window.globalOrbitAngle = (window.globalOrbitAngle + Math.PI * 2 * 50 / 120000) % (Math.PI * 2)
    }
    ...
    // Gated on visibility: this widget is cached (hidden, not destroyed) by
    // Main.qml, and isTodaySelected is true by default — ungated, this spawned
    // a `cat` + full JSON re-parse EVERY SECOND, forever, for a hidden widget.
    // onVisibleChanged below refreshes immediately on reopen, so the visible
    // data is never staler than it was before.
    Timer {
        interval: 1000
    ...
    onVisibleChanged: if (visible) requestDataUpdate()
