...
Item {
    ...
    // The cache is only ever READ once (at creation, above), so persist the
    // last-seen stats when the popup closes instead of 13 QSettings writes per
    // 1.5s poll tick while it is open.
    function flushWidgetCache() {
        widgetCache.cpuUsage = cpuUsage;
        widgetCache.ramUsage = ramUsage;
        widgetCache.diskUsage = diskUsage;
        widgetCache.cpuTemp = cpuTemp;
        widgetCache.gpuUsage = gpuUsage;
        widgetCache.gpuVram = gpuVram;
        widgetCache.gpuTemp = gpuTemp;
        widgetCache.powerProfile = powerProfile;
        widgetCache.upHours = upHours;
        widgetCache.upMins = upMins;
        widgetCache.sysVolume = sysVolume;
        widgetCache.sysMuted = sysMuted;
    }
    onVisibleChanged: {
        if (visible) {
            // The focus countdown timer is gated on visibility; refresh the
            // remaining time immediately so the first visible frame is current.
            if (focusMode !== "default" && focusEndEpoch > 0) refreshFocusCountdown();
        } else {
            flushWidgetCache();
        }
    }
    ...
    // System storage used % (0–100) — drawn as a ring around the central data circle.
    ...
    // Holds the poll's volume writes off for 800ms after the last drag movement
    // (no triggeredOnStart — that reset the flag instantly and defeated the hold).
    ...
    // Username straight from the environment — no bash round-trip needed.
    Component.onCompleted: {
        currentUserName = (Quickshell.env("USER") || "").trim();
        widgetCache.currentUserName = currentUserName;
    }
    ...
    Process {
        ...
        command: ["bash", "-c",
            // 0 cpu% needs `vmstat 1 2` (~1.0s inherent) — start it in the background on
            // fd3, gather everything else meanwhile (buffered into REST so line order is
            // unchanged), then emit cpu% first. Wall time ~1.05s vs ~1.9s serial, so the
            // poll always finishes before the 1.5s re-arm instead of racing it.
            "exec 3< <(vmstat 1 2 | tail -1 | awk '{print 100 - $15}' || echo '0'); " +
            "SENS=$(sensors 2>/dev/null); " +                                                                    // one sensors snapshot reused for CPU temp and GPU temp
            "command -v nvidia-smi >/dev/null 2>&1 && HAS_NV=1 || HAS_NV=; " +                                   // probe nvidia-smi ONCE instead of forking it per metric
            "dp=$(df / 2>/dev/null | awk 'NR==2 {print $5}' | tr -d '%'); " +                                    // df / computed once, reused for line 2 (disk%) and line 14 (sys storage)
            "REST=$(" +
            "free -m | awk '/Mem:/ {print int($3/$2 * 100)}' || echo '0'; " +                                    // 1 ram%
            "[ -n \"$dp\" ] && echo \"$dp\"; " +                                                                 // 2 disk%
            // 3 CPU temp (°C)
            "ctemp=$(grep -m 1 -E 'Package id 0|Tctl|Tdie' <<< \"$SENS\" | grep -oE '\\+[0-9]+\\.[0-9]+' | head -n1 | tr -d '+' | cut -d. -f1); [ -z \"$ctemp\" ] && ctemp=$(cat /sys/class/thermal/thermal_zone*/temp 2>/dev/null | head -n1 | awk '{print int($1/1000)}'); echo \"${ctemp:-0}\"; " +
            ...
            "cat /sys/class/drm/card*/device/gpu_busy_percent 2>/dev/null | head -1 || { if [ -n \"$HAS_NV\" ]; then nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader,nounits 2>/dev/null | grep -E '^[0-9]+$' || echo '0'; else echo '0'; fi; }; " + // 7 gpu%
            // 8 GPU VRAM percent: try amdgpu sysfs (used/total bytes), then nvidia-smi.
            ...
            "elif [ -n \"$HAS_NV\" ]; then nvidia-smi --query-gpu=memory.used,memory.total --format=csv,noheader,nounits 2>/dev/null | awk -F', ' 'NR==1{if($2>0) print int($1*100/$2); else print 0}' || echo '0'; fi; " +
            // 9 GPU temp (°C): amdgpu edge sensor, then nvidia-smi.
            "gtemp=$(grep -m1 -E 'edge|junction' <<< \"$SENS\" | grep -oE '\\+[0-9]+\\.[0-9]+' | head -n1 | tr -d '+' | cut -d. -f1); [ -z \"$gtemp\" ] && [ -n \"$HAS_NV\" ] && gtemp=$(nvidia-smi --query-gpu=temperature.gpu --format=csv,noheader,nounits 2>/dev/null | head -1); echo \"${gtemp:-0}\"; " +
            // 10 network cumulative bytes: sum rx and tx across real interfaces.
            ...
            // 11 system storage used %: root filesystem.
            "[ -n \"$dp\" ] && echo \"$dp\"; " +
            "); " +
            "IFS= read -r cpu <&3 && echo \"$cpu\"; exec 3<&-; printf '%s\\n' \"$REST\""
        ]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (lines.length >= 7) {
                    ...
                    if (lines.length >= 9) {
                        window.gpuVram = parseInt(lines[8]) || 0;
                    }
                    if (lines.length >= 10) {
                        window.gpuTemp = parseInt(lines[9]) || 0;
                    }
                    if (lines.length >= 11) {
                        let nb = (lines[10] || "0 0").trim().split(" ");
                    ...
                    // Storage-ring data (line 11). Clamp to 0–100.
                    function clampPct(v) { v = parseInt(v) || 0; return v < 0 ? 0 : (v > 100 ? 100 : v); }
                    if (lines.length >= 12) {
                        window.sysStoragePct = clampPct(lines[11]);
                    }
                }
            }
        }
    ...
    Process {
        id: maintAutoGate
        command: ["bash","-c","M=~/.cache/qs_last_auto_update; N=$(date +%s); L=$(cat \"$M\" 2>/dev/null || echo 0); L=${L//[^0-9]/}; L=${L:-0}; if [ $((N-L)) -gt 1036800 ]; then echo \"$N\" > \"$M\"; echo go; fi"]
    ...
    function refreshFocusCountdown() {
    ...
    Timer {
        ...
        // Gated on visibility (this widget is cached forever while hidden);
        // onVisibleChanged refreshes the countdown the moment the popup opens.
        interval: 1000; running: window.visible && window.focusMode !== "default" && window.focusEndEpoch > 0; repeat: true
        onTriggered: window.refreshFocusCountdown()
    }
    ...
    // Ambient blob orbit. As a per-frame NumberAnimation this repainted the whole
    // scene at 144Hz (~10% of a core). The blobs sweep 2π per 90s, so a 20fps tick
    // moves them well under a pixel per step at 6-8% opacity — same angular
    // velocity, same path, at a capped repaint rate.
    Timer {
        interval: 50; repeat: true; running: window.visible
        onTriggered: window.globalOrbitAngle = (window.globalOrbitAngle + Math.PI * 2 * 0.05 / 90) % (Math.PI * 2)
    }
            ...
            RowLayout {
                                        ...
                                        MouseArea {
                                            ...
                                            RowLayout {
                                                ...
                                                Text {
                                                    ...
                                                    Layout.fillWidth: true
                                                    verticalAlignment: Text.AlignVCenter
                                                    // appName comes from the D-Bus sender; never parse as markup.
                                    ...
                                    ColumnLayout {
                                        ...
                                        RowLayout {
                                            ...
                                            Text {
                                                ...
                                                Layout.fillWidth: true
                                                wrapMode: Text.Wrap
                                                // D-Bus senders control this string; never parse it as markup.
                                                textFormat: Text.PlainText
                                            }
                ...
                Item {
                    ...
                    Item {
                        ...
                        // Quantized to whole minutes: the bar moves ~0.0003px per second,
                        // so per-second updates only served to retrigger the 800ms width
                        // Behavior continuously and keep the render loop awake.
                        property real frac: Math.min(1, (window.uptimeSec - (window.uptimeSec % 60)) / twoWeeks)
                    ...
                    Grid {
                        ...
                        Item {
                            ...
                            // Timer-stepped 400ms OutQuint sweep (16ms steps): a continuous
                            // NumberAnimation kept the 144Hz render loop awake for 1200ms of
                            // every 1.5s poll tick (~10% of a core across the four orbs).
                            // Stepping by timer wakes the loop ~25 times per sweep instead.
                            // The initial binding seeds the cached value, then onCompleted
                            // breaks it so only the sweep timer writes animVal.
                            property real animVal: window.cpuUsage
                            Component.onCompleted: animVal = animVal
                            property real sweepFrom: 0
                            property real sweepT0: 0
                            property real sweepTarget: window.cpuUsage
                            onSweepTargetChanged: { sweepFrom = animVal; sweepT0 = Date.now(); cpuSweep.restart(); }
                            Timer {
                                id: cpuSweep; interval: 16; repeat: true
                                onTriggered: {
                                    var t = Math.min(1, (Date.now() - cpuOrb.sweepT0) / 400);
                                    var e = 1 - Math.pow(1 - t, 5);
                                    cpuOrb.animVal = cpuOrb.sweepFrom + (cpuOrb.sweepTarget - cpuOrb.sweepFrom) * e;
                                    if (t >= 1) stop();
                                }
                            }
                            // Repaint on quarter-percent steps (<1px of arc).
                            property int paintVal: Math.round(animVal * 4)
                            onPaintValChanged: cpuCanvas.requestPaint()
                        ...
                        Item {
                            ...
                            property real sweepFrom: 0
                            property real sweepT0: 0
                            property real sweepTarget: window.ramUsage
                            onSweepTargetChanged: { sweepFrom = animVal; sweepT0 = Date.now(); ramSweep.restart(); }
                            Timer {
                                id: ramSweep; interval: 16; repeat: true
                                onTriggered: {
                                    var t = Math.min(1, (Date.now() - ramOrb.sweepT0) / 400);
                                    var e = 1 - Math.pow(1 - t, 5);
                                    ramOrb.animVal = ramOrb.sweepFrom + (ramOrb.sweepTarget - ramOrb.sweepFrom) * e;
                                    if (t >= 1) stop();
                                }
                            }
                            property int paintVal: Math.round(animVal * 4)
                            onPaintValChanged: ramCanvas.requestPaint()
                        ...
                        Item {
                            ...
                            property real sweepFrom: 0
                            property real sweepT0: 0
                            property real sweepTarget: window.gpuUsage
                            onSweepTargetChanged: { sweepFrom = animVal; sweepT0 = Date.now(); gpuSweep.restart(); }
                            Timer {
                                id: gpuSweep; interval: 16; repeat: true
                                onTriggered: {
                                    var t = Math.min(1, (Date.now() - gpuOrb.sweepT0) / 400);
                                    var e = 1 - Math.pow(1 - t, 5);
                                    gpuOrb.animVal = gpuOrb.sweepFrom + (gpuOrb.sweepTarget - gpuOrb.sweepFrom) * e;
                                    if (t >= 1) stop();
                                }
                            }
                            property int paintVal: Math.round(animVal * 4)
                            onPaintValChanged: gpuCanvas.requestPaint()
                        ...
                        Item {
                            ...
                            property real sweepFrom: 0
                            property real sweepT0: 0
                            property real sweepTarget: window.gpuVram
                            onSweepTargetChanged: { sweepFrom = animVal; sweepT0 = Date.now(); tempSweep.restart(); }
                            Timer {
                                id: tempSweep; interval: 16; repeat: true
                                onTriggered: {
                                    var t = Math.min(1, (Date.now() - tempOrb.sweepT0) / 400);
                                    var e = 1 - Math.pow(1 - t, 5);
                                    tempOrb.animVal = tempOrb.sweepFrom + (tempOrb.sweepTarget - tempOrb.sweepFrom) * e;
                                    if (t >= 1) stop();
                                }
                            }
                            property int paintVal: Math.round(animVal * 4)
                            onPaintValChanged: tempCanvas.requestPaint()
                    ...
                    // ── Storage ring ──────────────────────────────────────────
                    // PC storage used % as a circular bar around the central data
                    // circle, in the same track+gradient style as the orb rings.
                    Canvas {
                        id: sideArcs
                        anchors.fill: sysGrid
                        z: 4
                        ...
                        onPaint: {
                            var ctx = getContext("2d"); ctx.clearRect(0, 0, width, height);
                            var cxC = window.s(177.5), cyC = window.s(177.5);
                            var rr = window.s(31);
                            ctx.lineCap = "round";
                            ctx.lineWidth = window.s(3);
                            ctx.beginPath(); ctx.arc(cxC, cyC, rr, 0, 2 * Math.PI);
                            ctx.strokeStyle = window.surface0.toString(); ctx.stroke();
                            var p = Math.min(100, Math.max(0, sysSt)) / 100;
                            if (p > 0) {
                                var a0 = -Math.PI / 2;   // fill clockwise from 12 o'clock
                                var grad = ctx.createLinearGradient(cxC - rr, cyC + rr, cxC + rr, cyC - rr);
                                grad.addColorStop(0, window.green.toString());
                                grad.addColorStop(1, window.teal.toString());
                                ctx.lineWidth = window.s(4.5);
                                ctx.beginPath(); ctx.arc(cxC, cyC, rr, a0, a0 + p * 2 * Math.PI);
                                ...
                                Row {
                                    ...
                                    Repeater {
                                        model: [
                                            ...
                                            { lbl: "Clean", act: "clean", col: window.sapphire, show: true },
