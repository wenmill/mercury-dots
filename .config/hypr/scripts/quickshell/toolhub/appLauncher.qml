...
Item {
    ...
    // tool slider is hidden (all three tools are shown side-by-side in the hub).
    ...
    // ── Dock apps (shared with the TopBar dock) ──────────────────────────────
    // Persisted in ~/.cache/qs_dock_apps.json as an array of {name,exec,icon}.
    // The TopBar dock reads the same file; add/remove via the right-click menu.
    property var dockApps: []
    function _dockIndex(execStr) {
        let key = window._cmdKey(execStr);
        for (let i = 0; i < window.dockApps.length; i++)
            if (window._cmdKey(window.dockApps[i].exec) === key) return i;
        return -1;
    }
    function _inDock(execStr) { return window._dockIndex(execStr) >= 0; }
    function saveDockApps() {
        let json = JSON.stringify(window.dockApps).replace(/'/g, "'\\''");
        Quickshell.execDetached(["bash", "-c",
            "mkdir -p ~/.cache && printf '%s' '" + json + "' > ~/.cache/qs_dock_apps.json"]);
    }
    function addToDock(name, execStr, icon) {
        if (window._inDock(execStr)) return;
        window.dockApps = window.dockApps.concat([{ name: name, exec: execStr, icon: icon }]);
        window.saveDockApps();
    }
    function removeFromDock(execStr) {
        let idx = window._dockIndex(execStr);
        if (idx < 0) return;
        window.dockApps = window.dockApps.filter((a, i) => i !== idx);
        window.saveDockApps();
    }
    Process {
        id: dockAppsReader; running: true
        command: ["bash", "-c", "cat ~/.cache/qs_dock_apps.json 2>/dev/null || echo '[]'"]
        stdout: StdioCollector { onStreamFinished: {
            try { window.dockApps = JSON.parse(this.text) || []; } catch(e) { window.dockApps = []; }
        } }
    }
    ...
    Process {
        id: appFetcher
        running: true
        command: ["bash", "-c", "python3 " + Quickshell.env("HOME") + "/.config/hypr/scripts/quickshell/toolhub/app_fetcher.py"]
    ...
    // Timer-stepped at 20fps (was a per-frame NumberAnimation); off entirely
    // when embedded — the hub draws the shared background blobs instead.
    property real globalOrbitAngle: 0
    Timer {
        interval: 50; repeat: true; running: window.visible && !window.embedded
        onTriggered: window.globalOrbitAngle = (window.globalOrbitAngle + Math.PI * 2 * 50 / 90000) % (Math.PI * 2)
    }
    ...
    Rectangle {
        ...
        // Fill the popup window so the registry size (560×500) applies, matching the
        // tools and matrix popups (which use anchors.fill). The app list below uses
        // Layout.fillHeight to take all remaining space.
        anchors.fill: parent

        radius: window.s(16)
        // Embedded in the hub → a translucent card over the hub's shared
        // calendar-style background; standalone → the original opaque window.
        color: window.embedded ? Qt.rgba(window.surface0.r, window.surface0.g, window.surface0.b, 0.2)
                               : Qt.rgba(window.base.r, window.base.g, window.base.b, 1.0)
        border.color: window.embedded ? Qt.rgba(window.surface1.r, window.surface1.g, window.surface1.b, 0.4)
                                      : window.surface1
        border.width: 1
        clip: true
        ...
        MouseArea {
            ...
            onClicked: function(m) {
                m.accepted = true;
                Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/hypr/scripts/qs_manager.sh", "open", "toolhub"]);
            }
        }

        // --- AMBIENT BLOBS ---
        Rectangle {
            visible: !window.embedded   // the hub supplies the shared blobs
                ...
                delegate: Item {
                    ...
                    Rectangle {
                        ...
                        MouseArea {
                            ...
                            onClicked: function(mouse) {
                                if (mouse.button === Qt.RightButton) {
                                    ...
                                    appAssignMenu.appName = model.name;
                                    appAssignMenu.appExec = model.exec;
                                    appAssignMenu.appIcon = model.icon;
                                    appAssignMenu.popup();
                                    return;
                                }
                        ...
                        Menu {
                            ...
                            MenuItem {
                                text: window._inDock(appAssignMenu.appExec) ? "Remove from Dock" : "Add to Dock"
                                onTriggered: {
                                    if (window._inDock(appAssignMenu.appExec)) window.removeFromDock(appAssignMenu.appExec);
                                    else window.addToDock(appAssignMenu.appName, appAssignMenu.appExec, appAssignMenu.appIcon);
                                }
                            }
                            MenuSeparator {}
                            MenuItem {
