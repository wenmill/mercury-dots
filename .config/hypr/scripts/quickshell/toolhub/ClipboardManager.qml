...
Item {
    ...
    // tool slider is hidden and the search field doesn't grab focus (the launcher
    // column owns initial keyboard focus in the hub).
    ...
    Process {
        id: clipFetcher
        running: true
        command: ["python3", Quickshell.env("HOME") + "/.config/hypr/scripts/quickshell/toolhub/clip_fetcher.py", window.currentOffset, window.fetchLimit, paths.getCacheDir("clipboard")]
    ...
    function loadMore() {
        ...
        clipFetcher.command = ["python3", Quickshell.env("HOME") + "/.config/hypr/scripts/quickshell/toolhub/clip_fetcher.py", window.currentOffset, window.fetchLimit, paths.getCacheDir("clipboard")];
    ...
    Connections {
        ...
        function onVisibleChanged() {
            if (window.visible) {
                ...
                clipFetcher.command = ["python3", Quickshell.env("HOME") + "/.config/hypr/scripts/quickshell/toolhub/clip_fetcher.py", 0, window.fetchLimit, paths.getCacheDir("clipboard")];
                clipFetcher.running = true;
            } else {
                searchInput.text = "";
                window.pendingIndex = -1;
    ...
    Timer {
        // Timer-stepped at 20fps; off when embedded (hub draws the shared blobs).
    ...
    Rectangle {
        ...
        // tools and matrix popups. The grid below anchors to the bottom to take all
        // remaining space.
        anchors.fill: parent
