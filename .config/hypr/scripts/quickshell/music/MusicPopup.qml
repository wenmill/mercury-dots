...
Item {
    ...
    // The gradient flow, orbit blobs, conic sweep and vinyl spin were four
    // separate per-frame animations pinning the render loop at full frame rate
    // whenever the popup was open. One 50ms wall-clock timer drives them all;
    // every step is at or below the perception floor (the fastest motion is
    // the 8s gradient flow at ~6px per step on a soft gradient).
    property real ambientPhase: 0
    property real _ambientLast: 0
    property real vinylAngle: 0
    Timer {
        interval: 50; repeat: true; running: root.visible
        onTriggered: {
            var now = Date.now();
            var dt = root._ambientLast > 0 ? Math.min(200, now - root._ambientLast) : 0;
            root._ambientLast = now;
            root.ambientPhase += dt;
            // Vinyl: 360°/8s, advancing ONLY while playing — exactly the old
            // paused-in-place behaviour of the paused: property.
            if (root.musicData.status === "Playing")
                root.vinylAngle = (root.vinylAngle + 360 * dt / 8000) % 360;
        }
        onRunningChanged: root._ambientLast = 0
    }
    readonly property real catppuccinFlowOffset: (ambientPhase % 8000) / 8000
    ...
    function execCmd(cmdStr) {
        // Fire-and-forget without constructing QML/JS from the string: the old
        // Qt.createQmlObject + template-literal version escaped backticks but
        // not ${}, so a command containing "${...}" executed arbitrary JS.
    ...
    // The EQ state file only changes through this popup's own actions (which
    // update the UI optimistically) — the old 2Hz confirm poll was pure
    // process-spawn overhead riding the music timer.
    Timer {
        interval: 2500
        running: root.visible
        repeat: true
                ...
                if (this.text) {
                    ...
                    if (outStr.length > 0) {
                        try {
                            ...
                            // Skip identical payloads: replacing the whole object
                            // re-evaluates EVERY binding in the popup and fired all
                            // the colour/opacity Behaviors twice a second even when
                            // nothing was playing and nothing had changed.
                            if (JSON.stringify(newData) !== JSON.stringify(root.musicData))
        ...
        Item {
            ...
            Item {
                ...
                Rectangle {
                    ...
                    rotation: (root.ambientPhase % 5000) / 5000 * 360
        ...
        Rectangle {
            ...
            Item {
                ...
                Image {
                    ...
                    sourceSize.width: 1024; sourceSize.height: 576
                        ...
                        Rectangle {
                            ...
                            Item {
                                ...
                                Image {
                                    ...
                                    sourceSize.width: 600; sourceSize.height: 600
                                    fillMode: Image.PreserveAspectCrop
                                    visible: false 
                                }
                                Rectangle {
                            ...
                            Slider {
                                ...
                                onPressedChanged: {
                                    if (pressed) {
                                        ...
                                        // Argument-passed: the MPRIS player name is
                                        // externally claimable — never splice it into a
                                        // shell string.
                                        var safePlayer = root.musicData.playerName ? String(root.musicData.playerName) : "";
                                        Quickshell.execDetached([Quickshell.env("HOME") + "/.config/hypr/scripts/quickshell/music/player_control.sh",
                                                                 "seek", value.toFixed(2), String(root.musicData.length), safePlayer]);
