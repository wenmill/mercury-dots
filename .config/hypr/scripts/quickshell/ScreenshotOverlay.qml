...
PanelWindow {
    ...
    // Debounced: the sliders fire per value tick while dragging, which used to
    // fork a bash per pixel of drag. Content travels as argv (micDevice is an
    // external string from pactl).
    Timer {
        id: audioPrefsSave
        interval: 300; repeat: false
        onTriggered: root._writeAudioPrefs()
    }
    function saveAudioPrefs() { audioPrefsSave.restart() }
    function _writeAudioPrefs() {
        let data = `${deskVol},${deskMute},${micVol},${micMute},${micDevice}`
        Quickshell.execDetached(["bash", "-c", 'printf %s "$1" > "$2"', "qs-audio",
                                 data, paths.getStateDir("screenshot") + "/audio_prefs"])
    }
    // Flush any pending debounced write before the overlay process exits.
    function quitOverlay() {
        if (audioPrefsSave.running) { audioPrefsSave.stop(); _writeAudioPrefs(); }
        Qt.quit()
    }
    ...
    // --- Keyboard Shortcuts ---
    Shortcut { sequence: "Escape"; onActivated: root.quitOverlay() }
    ...
    MouseArea {
        ...
        onPressed: (mouse) => {
            if (mouse.button === Qt.RightButton) { root.quitOverlay(); return; }
            if (root.isVideoMode) return; 
            ...
            ListView {
                ...
                delegate: Rectangle {
                    ...
                    RowLayout {
                        ...
                        Text { text: model.devDesc; textFormat: Text.PlainText; color: root.micDevice === model.devName ? _theme.mauve : _theme.text; font.pixelSize: s(12); elide: Text.ElideRight; Layout.fillWidth: true }
                    }
                    MouseArea { 
        ...
        Row {
            ...
            Item {
                ...
                Row {
                    ...
                    ToolbarBtn { 
                        anchors.verticalCenter: parent.verticalCenter
                        iconTxt: "󰅖"; isDanger: true; onClicked: root.quitOverlay()
                    }
                }
            }
        }
    ...
    Repeater {
        ...
        delegate: Rectangle {
            ...
            RowLayout {
                ...
                Text {
                    text: model.qText
                    textFormat: Text.PlainText   // QR content is attacker-controlled
                ...
                ToolbarBtn {
                    ...
                    iconTxt: "󰆏"
                    onClicked: {
                        // argv, not splice: QR text is arbitrary external data
                        // (and echo would parse a leading -e/-n as flags)
                        Quickshell.execDetached(["bash", "-c", 'printf %s "$1" | wl-copy', "qs-qr", model.qText]);
                        root.showQrPopup = false;
                    }
                }
    ...
    function performQrScan() {
        ...
        Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/hypr/scripts/screenshot.sh",
                                 "--geometry", root.geometryString, "--scan-qr"])
        qrWaitTimer.start()
    }
    ...
    // argv all the way down — micDevice is external (pactl) and used to ride
    // inside a double-quoted spliced command string.
    function executeCapture(openEditor, isRecord) {
        let args = ["bash", Quickshell.env("HOME") + "/.config/hypr/scripts/screenshot.sh",
                    "--geometry", root.geometryString]
        if (isRecord) {
            args.push("--record",
