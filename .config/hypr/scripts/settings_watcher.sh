...
# reload.sh's orphan sweep kills our inotifywait (pattern matches .config/hypr),
# which ends the read loop and exits this script — reload.sh respawns us, and
# this guard makes that respawn idempotent.
...
# (was scripts/weather.sh — wrong path since the calendar move, so the .env
# trigger below silently never refreshed weather)
...
# Refuse to regenerate from a missing, empty, or invalid settings.json.
# A bad read here would otherwise blank out autostart.conf (and friends),
# which leaves the next login with no shell/wallpaper — a grey screen.
# Returning non-zero makes compile_settings abort and leave existing configs intact.
validate_settings() {
    if [ ! -s "$SETTINGS_FILE" ]; then
        echo "ERROR: $SETTINGS_FILE is missing or empty; refusing to regenerate configs." >&2
        return 1
    fi
    if ! jq empty "$SETTINGS_FILE" >/dev/null 2>&1; then
        echo "ERROR: $SETTINGS_FILE is not valid JSON; refusing to regenerate configs." >&2
        return 1
    fi
    return 0
}
...
compile_settings() {
    ...
    # Guard: never overwrite working configs from a broken settings.json.
    if ! validate_settings; then
        echo "Aborting regeneration — existing configs left untouched." >&2
        command -v notify-send >/dev/null 2>&1 && \
            notify-send -u critical "Hyprland settings" \
                "settings.json is empty or invalid — config regeneration skipped, existing config preserved." 2>/dev/null
        return 1
    fi
...
# Propagate the failure code so callers know a bad settings.json was rejected.
if [[ "$1" == "--compile" ]]; then
    compile_settings
    exit $?
fi
